...
const replicateModel = `${process.env.UPSCALING_API_REPLICATE_MODEL || ""}`
const replicateModelVersion = `${process.env.UPSCALING_API_REPLICATE_MODEL_VERSION || ""}`

export async function upscaleVideoToURL(base64mp4: string): Promise<string> {
  if (!replicateToken) {
    throw new Error(`you need to configure your AUTH_REPLICATE_API_TOKEN in order to use interpolation`)
  }
  if (!replicateModel) {
    throw new Error(`you need to configure your UPSCALING_API_REPLICATE_MODEL in order to use interpolation`)
  }

  if (!replicateModelVersion) {
    throw new Error(`you need to configure your UPSCALING_API_REPLICATE_MODEL_VERSION in order to use interpolation`)
  }
