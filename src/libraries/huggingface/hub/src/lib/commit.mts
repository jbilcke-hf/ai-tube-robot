import { isFrontend, base64FromBytes } from "../../../shared/index.mts";
import { HUB_URL } from "../consts.mts";
import { HubApiError, createApiError, InvalidApiResponseFormatError } from "../error.mts";
import type {
	ApiCommitHeader,
	ApiCommitLfsFile,
	ApiCommitOperation,
...
} from "../types/api/api-commit.d.ts";
import type { Credentials, RepoDesignation } from "../types/public.d.ts";
import { checkCredentials } from "../utils/checkCredentials.mts";
import { chunk } from "../utils/chunk.mts";
import { promisesQueue } from "../utils/promisesQueue.mts";
import { promisesQueueStreaming } from "../utils/promisesQueueStreaming.mts";
import { sha256 } from "../utils/sha256.mts";
import { toRepoId } from "../utils/toRepoId.mts";
import { WebBlob } from "../utils/WebBlob.mts";
import { createBlob } from "../utils/createBlob.mts";
import { eventToGenerator } from "../utils/eventToGenerator.mts";
				...
				return promisesQueue(
					operations.map((op) => async () => {
						...
						do {
							...
							if (!res.done) {
								yieldCallback({ event: "fileProgress", path: op.path, progress: res.value as any, state: "hashing" });
							}
						} while (!res.done);
						const sha = res.value;
