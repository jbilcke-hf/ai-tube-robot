import { isFrontend } from "../../../shared/index.mts";
import { eventToGenerator } from "./eventToGenerator.mts";
import { hexFromBytes } from "./hexFromBytes.mts";
...
export async function* sha256(
	...
	if (!cryptoModule) {
		cryptoModule = await import("./sha256-node.mts");
	}

	return yield* cryptoModule.sha256Node(buffer, { abortSignal: opts?.abortSignal });
}

let cryptoModule: typeof import("./sha256-node.mts");
